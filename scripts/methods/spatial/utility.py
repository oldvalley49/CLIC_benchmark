...
def feature_selection(var_genes, tissue, cor_method, var_num, total_num):
    metadata = pd.read_csv("data/raw_data/rna_atac/metadata.csv", index_col=0)
    species = metadata.loc[tissue, "species"]
    var_genes = set(var_genes)
    ...
    if var_num==total_num:
        return (var_genes, 'Inf')
    for gene in corr.index:
        cor_num += 1
        if gene in var_genes:
            selected_genes.add(gene)
        if len(selected_genes.intersection(var_genes))==total_num:
            break
    return (selected_genes.intersection(var_genes), cor_num)
