...
# use.features = intersect(VariableFeatures(atac), VariableFeatures(rna))
...
dir.create(file.path("plots/paired/rna_atac", tissue, "umap"), showWarning=FALSE)
ggsave(file.path("plots/paired/rna_atac", tissue, "umap",paste0(run.id, ".jpeg")), plot=p)
# output coembed
...
dir.create(file.path("plots/paired/rna_atac", tissue,"annotation_accuracy"), showWarning=FALSE)
