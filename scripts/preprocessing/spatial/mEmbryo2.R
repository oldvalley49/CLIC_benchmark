...
rna_counts <- readMM("data/raw_data/rna_atac/spatial/mEmbryo2/RNA/E13_50_um_3/matrix.mtx")
barcodes <- read.table(file="data/raw_data/rna_atac/spatial/mEmbryo2/RNA/E13_50_um_3/barcodes.tsv", sep='\t')
genes <- read.table(file='data/raw_data/rna_atac/spatial/mEmbryo2/RNA/E13_50_um_3/features.tsv', sep='\t')
colnames(rna_counts) <- barcodes$V1
rownames(rna_counts) <- genes$V2
rna_counts <- as(rna_counts, "dgCMatrix")

frags <- "data/raw_data/rna_atac/spatial/mEmbryo2/GSM8189706_ME13_50um_3_ATAC.fragments.tsv.gz"
fragments = CreateFragmentObject(frags)
peaks <- CallPeaks(fragments)
cells <- colnames(rna_counts)
atac_counts = FeatureMatrix(fragments = fragments, features=peaks, cells=cells)
...
out_dir <- "data/processed_data/spatial/mEmbryo2/"
