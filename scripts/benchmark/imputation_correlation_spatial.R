...
truth <- readRDS(glue("data/processed_data/spatial/{tissue}/norm_rna_counts.RDS"))
imputed <- readRDS(glue("output/spatial/{tissue}/geximpute/{method}"))
...
dir.create(glue("output/spatial/{tissue}/impute_corr/"), showWarnings=FALSE)
